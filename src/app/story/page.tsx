...
import { storyChapters, storyIntro, storyThreads } from "@/content/story";
            ...
            {storyChapters.map((chapter, index) => {
              ...
              return (
                <li
                  ...
                  id={chapter.id}
                  data-chapter
                  className="story-chapter relative scroll-mt-28 py-14 first:pt-4 sm:py-16"
                >
                  {/* Node on the spine, lit by this chapter's own --p and
                      centred on the kicker line. The first chapter runs a
                      shorter top padding, so its node sits higher. */}
                  <span
                    aria-hidden
                    className={`d-step absolute -left-8 h-2.5 w-2.5 -translate-x-[4.5px] rounded-full bg-fg sm:-left-14 ${
                      index === 0 ? "top-[1.15rem]" : "top-[3.65rem]"
                    }`}
                  ...
                  <div className="grid gap-y-8 lg:grid-cols-12 lg:gap-x-10">
                    <div className="lg:col-span-7">
                      ...
                      <h2 className="mt-4 text-[1.75rem] leading-[1.1] tracking-[-0.02em] sm:text-[2.25rem]">
                        {pre}
                        {accent ? (
                      ...
                      {chapter.paragraphs.map((paragraph, i) => (
                        <p
                          ...
                          className={`max-w-[58ch] text-[0.9375rem] leading-relaxed ${
                            i === 0 ? "mt-6 text-muted" : "mt-4 text-muted"
                          }`}
                        >
                          {paragraph}
                        </p>
